...
if (!uri) {
  console.error(
    "MONGODB_URI not set. Run with: node --env-file=.env.local scripts/update-branislava-nikola-josnicka-banja.mjs",
  );
  process.exit(1);
}

const SLUG = "branislava-nikola";
const UPDATES = {
  premium_city: "josnicka_banja",
};
...
try {
  ...
  const before = await couples.findOne(
    { slug: SLUG },
    { projection: { slug: 1, premium_city: 1, _id: 0 } },
  );
  if (!before) {
