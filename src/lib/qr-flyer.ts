...
export async function generateQrFlyerPDF(input: QrFlyerInput): Promise<void> {
  const { eventName, url, title, lines, bottom, filename } = input;
  ...
  // Background
  doc.setFillColor(...blend(PRIMARY, 0.08));
  doc.rect(0, 0, W, H, "F");
  ...
  doc.text(eventName, cx, 27, { align: "center" });

  // Divider
  doc.setDrawColor(...blend(PRIMARY, 0.3));
  doc.setLineWidth(0.3);
  doc.line(cx - 15, 34, cx + 15, 34);

  // Title
  doc.setFont("Serif");
  doc.setFontSize(14);
  doc.setTextColor(...TEXT);
  doc.text(title, cx, 43, { align: "center" });
